...
public class ScheduleTypeController {
    ...
    public ResponseEntity<CommonSuccessDto> list(Pageable pageable) {
        Page<ScheduleTypeDetailDto> result = scheduleTypeService.list(pageable);
        return ResponseEntity.ok(
                CommonSuccessDto.builder()
                        .result(result)
                        .status_code(HttpStatus.OK.value())
                        .status_message("스케줄 종류 목록 조회 완료")
                        .build()
        );
    }
    ...
    public ResponseEntity<CommonSuccessDto> detail(@PathVariable Long id) {
        ScheduleTypeDetailDto result = scheduleTypeService.detail(id);
        return ResponseEntity.ok(
                CommonSuccessDto.builder()
                        .result(result)
                        .status_code(HttpStatus.OK.value())
                        .status_message("스케줄 종류 상세 조회 완료")
                        .build()
        );
    }
    ...
    public ResponseEntity<CommonSuccessDto> create(@Valid @RequestBody ScheduleTypeCreateDto req) {
        ScheduleTypeDetailDto result = scheduleTypeService.create(req);
        return ResponseEntity.status(HttpStatus.CREATED).body(
                CommonSuccessDto.builder()
                        .result(result)
                        .status_code(HttpStatus.CREATED.value())
                        .status_message("스케줄 종류 생성 완료")
                        .build()
        );
    }
    ...
    public ResponseEntity<CommonSuccessDto> update(@PathVariable Long id,
                                                   @Valid @RequestBody ScheduleTypeUpdateDto req) {
        ScheduleTypeDetailDto result = scheduleTypeService.update(id, req);
        return ResponseEntity.ok(
                CommonSuccessDto.builder()
                        .result(result)
                        .status_code(HttpStatus.OK.value())
                        .status_message("스케줄 종류 수정 완료")
                        .build()
        );
    }
