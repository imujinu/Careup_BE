...
public class EmployeeService {
    ...
    private record Auth(Long employeeId, String role) { boolean isHqAdmin() { return "HQ_ADMIN".equals(role); } }
    ...
    private void safeDeleteOldImage(String url) {
        try { awsS3Uploader.deleteByUrl(url); } catch (Exception ignored) { }
    }
    ...
    private List<EmployeeDispatchDto> saveAllDispatches(Employee employee, List<DispatchAssignmentDto> dispatches) {
        ...
        for (DispatchAssignmentDto d : dispatches) {
            ...
            if (overlapped) {
                throw new IllegalArgumentException("동일 지점에 겹치는 배치가 있습니다.");
            }
