...
public interface PurchaseOrderRepository extends JpaRepository<PurchaseOrder, Long> {
    ...
    // JOIN FETCH를 사용한 발주 목록 조회 (N+1 문제 해결)
    @Query("SELECT DISTINCT po FROM PurchaseOrder po " +
            "LEFT JOIN FETCH po.orderDetails " +
            "WHERE po.branchId = :branchId")
    List<PurchaseOrder> findByBranchIdWithDetails(@Param("branchId") Long branchId);

    // JOIN FETCH를 사용한 전체 발주 목록 조회 (N+1 문제 해결)
    @Query("SELECT DISTINCT po FROM PurchaseOrder po " +
            "LEFT JOIN FETCH po.orderDetails " +
            "WHERE po.orderStatus != :cancelledStatus")
    List<PurchaseOrder> findAllWithDetails(@Param("cancelledStatus") OrderStatus cancelledStatus);
