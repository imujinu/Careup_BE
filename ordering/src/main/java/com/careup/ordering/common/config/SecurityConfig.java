...
import com.careup.ordering.common.auth.JwtAccessDeniedHandler;        /// 403 핸들러
import com.careup.ordering.common.auth.JwtAuthenticationEntryPoint;   /// 401 핸들러
import com.careup.ordering.common.auth.JwtTokenFilter;                /// JWT 필터
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http,
        ...
        /// 1) CSRF 비활성화(세션/폼로그인 안 쓰는 API 서버)
        ...
        http.authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(
                        "/actuator/**",
                        "/public/**",            // 회원가입/중복체크 등 공개 API
                        "/auth/customer/**",     // 고객 전용 로그인/재발급/로그아웃
                        "/health"
                ).permitAll()
                /// 그 외 모든 경로는 인증(토큰) 필요
